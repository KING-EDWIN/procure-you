...
export default function GeneralManagerDashboard() {
  ...
  const gmName = "Grace Manager"; // You can make this dynamic if needed
  const handledForms = forms.filter(f => f.history.some((h: FormHistoryEntry) => h.role === 'gm'));
  ...
  const avgApprovalTime = (() => {
    const times = handledForms.map(f => {
      const gmEntry = f.history.find((h: FormHistoryEntry) => h.role === 'gm');
