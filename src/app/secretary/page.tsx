...
export default function SecretaryDashboard() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-white p-6 flex flex-col items-center">
      <div className="w-full max-w-3xl bg-white rounded-2xl shadow-xl p-8 border border-indigo-100 mt-8">
        <div className="flex items-center mb-4">
          {roleIcon}
          <div>
            <h1 className="text-2xl font-bold text-indigo-900">Hon. Secretary Dashboard</h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-indigo-100 rounded-xl p-4 flex flex-col items-center">
            <div className="text-2xl font-bold text-indigo-700">{inbox.length}</div>
            <div className="text-xs text-indigo-700">Awaiting Your Action</div>
          </div>
          <div className="bg-indigo-50 rounded-xl p-4 flex flex-col items-center">
            <div className="text-2xl font-bold text-indigo-500">{handledForms.length}</div>
            <div className="text-xs text-indigo-500">Total Handled</div>
          </div>
        ...
        <div className="space-y-3 mb-8">
          ...
          {inbox.map(form => (
            <div key={form.id} className="p-4 rounded-lg border border-indigo-200 bg-indigo-50 flex flex-col sm:flex-row sm:items-center sm:justify-between">
              <div>
                <div className="font-semibold text-indigo-900">{form.title}</div>
                <div className="text-xs text-gray-500">Type: {form.type}</div>
                <div className="text-xs text-gray-500">Created: {form.created}</div>
                <div className="text-xs text-gray-500">Status: <span className="inline-block px-2 py-0.5 rounded bg-indigo-200 text-indigo-800 text-xs">{form.status}</span></div>
                <ApprovalTrail form={form} />
              </div>
              <div className="mt-2 sm:mt-0 flex flex-col sm:items-end gap-2">
                <button className="bg-indigo-600 text-white px-3 py-1 rounded text-xs font-semibold hover:bg-indigo-700 mt-2" onClick={() => handleSign(form)}>View & Sign</button>
              </div>
            </div>
          ))}
        </div>
        {showModal && selectedForm && (
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 overflow-auto">
            <div className="bg-white rounded-xl shadow-2xl p-8 w-full max-w-2xl relative max-h-[90vh] overflow-y-auto">
              ...
              <h2 className="text-xl font-bold mb-4 text-indigo-900">Sign Form</h2>
              ...
              <button className="bg-indigo-600 text-white px-4 py-2 rounded font-semibold hover:bg-indigo-700 w-full" onClick={() => handleSendToNext(selectedForm.id)}>Sign & Send to Hon. Treasurer</button>
            </div>
          </div>
        )}
      </div>
