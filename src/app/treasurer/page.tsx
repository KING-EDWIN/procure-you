...
export default function TreasurerDashboard() {
  ...
  const handledForms = forms.filter(f => f.history.some((h: FormHistoryEntry) => h.role === 'treasurer'));
  ...
  const avgApprovalTime = (() => {
    const times = handledForms.map(f => {
      const treasurerEntry = f.history.find((h: FormHistoryEntry) => h.role === 'treasurer');
