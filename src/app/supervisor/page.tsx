...
const roleIcon = (
  <span className="inline-block bg-yellow-100 text-yellow-600 rounded-full p-2 mr-2">
...
export default function SupervisorDashboard() {
  ...
  // Only show forms where currentRole is 'supervisor'
  const inbox = forms.filter(f => f.currentRole === "supervisor");

  // Compute enhanced stats for supervisor
  const handledForms = forms.filter(f => f.history.some((h) => h.role === 'supervisor'));
  ...
  const avgApprovalTime = (() => {
    const times = handledForms.map(f => {
      const supervisorEntry = f.history.find((h) => h.role === 'supervisor');
  ...
  const handleSendToNext = (id: number) => {
    ...
    signAndSend(id, JSON.stringify({ name, signatureUrl }), "procurement", "Awaiting Procurement Officer Action");
    setShowModal(false);
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-yellow-50 to-white p-6 flex flex-col items-center">
      <div className="w-full max-w-3xl bg-white rounded-2xl shadow-xl p-8 border border-yellow-100 mt-8">
        <div className="flex items-center mb-4">
          {roleIcon}
          <div>
            <h1 className="text-2xl font-bold text-yellow-900">Supervisor Dashboard</h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-yellow-100 rounded-xl p-4 flex flex-col items-center">
            <div className="text-2xl font-bold text-yellow-700">{inbox.length}</div>
            <div className="text-xs text-yellow-700">Awaiting Your Action</div>
          </div>
          <div className="bg-yellow-50 rounded-xl p-4 flex flex-col items-center">
            <div className="text-2xl font-bold text-yellow-500">{handledForms.length}</div>
            <div className="text-xs text-yellow-500">Total Handled</div>
          </div>
          ...
          <div className="bg-yellow-200 rounded-xl p-4 flex flex-col items-center col-span-2 md:col-span-1">
            <div className="text-2xl font-bold text-yellow-800">{avgApprovalTime ? `${avgApprovalTime} days` : '--'}</div>
            <div className="text-xs text-yellow-800">Avg. Approval Time</div>
          </div>
        </div>
        ...
        <div className="space-y-3 mb-8">
          ...
          {inbox.map(form => (
            <div key={form.id} className="p-4 rounded-lg border border-yellow-200 bg-yellow-50 flex flex-col sm:flex-row sm:items-center sm:justify-between">
              <div>
                <div className="font-semibold text-yellow-900">{form.title}</div>
                <div className="text-xs text-gray-500">Type: {form.type}</div>
                <div className="text-xs text-gray-500">Created: {form.created}</div>
                <div className="text-xs text-gray-500">Status: <span className="inline-block px-2 py-0.5 rounded bg-yellow-200 text-yellow-800 text-xs">{form.status}</span></div>
        ...
        {showModal && selectedForm && (
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 overflow-auto">
            <div className="bg-white rounded-xl shadow-2xl p-8 w-full max-w-2xl relative max-h-[90vh] overflow-y-auto">
              ...
              <h2 className="text-xl font-bold mb-4 text-yellow-900">Sign Form</h2>
              ...
              <button className="bg-green-600 text-white px-4 py-2 rounded font-semibold hover:bg-green-700 w-full" onClick={() => handleSendToNext(selectedForm.id)}>Sign & Send to Procurement Officer</button>
            </div>
          </div>
        )}
      </div>
